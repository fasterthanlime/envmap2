...
Vector3 Camera::getPos() const
{
	...
	float cosTheta = ZMath::cos(ZMath::deg2rad(theta));
	float sinTheta = ZMath::sin(ZMath::deg2rad(theta));
	float cosPhi = ZMath::cos(ZMath::deg2rad(phi));
	float sinPhi = ZMath::sin(ZMath::deg2rad(phi));
...
void Camera::recalculateProjMatrix()
{
	...
	float ymax = zNear * ZMath::tan(ZMath::deg2rad(fovy) * 0.5f);
	float ymin = -ymax;
